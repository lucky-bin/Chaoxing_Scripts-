...
// @supportURL   https://api.gocos.cn/static/cxapi/js/cxscript.user.js
...
if (url == '/mycourse/studentstudy') {
    ...
    _self.layui.use('layer', function () {
        this.layer.open({ content: '拖动进度条、倍速播放、秒过会导致不良记录！题库在慢慢补充。', title: '超星网课助手提示', btn: '我已知悉', offset: 't', closeBtn: 0 });
    });
...
function beforeFind() {
    ...
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'https://api.gocos.cn/index.php/cxapi/cxtimu/notice',
...
function findAnswer() {
    ...
    GM_xmlhttpRequest({
        method: 'POST',
        url: 'https://api.gocos.cn/index.php/cxapi/cxtimu/getanswer',
        headers: {
