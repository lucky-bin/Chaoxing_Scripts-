// ==UserScript==
// @name           超星学习通网课助手(支持图片/自主收录题目)Ne-21
// @namespace      nawlgzs@163.com
// @version        1.2.2
// @description    多项功能集成 | 自动挂机看尔雅MOOC，支持自动切换任务点，章测作业自动答题提交 | 解除各类功能限制，开放自定义参数 
// @author         Ne-21
// @match          *://*.chaoxing.com/*
// @match          *://*.edu.cn/*
...
if (url == "/mycourse/studentstudy") {
    ...
    _self.layui.use("layer", function () {
        this.layer.open({content: '拖动进度条、倍速播放、秒过会导致不良记录！\n脚本版本：v1.2.2   云淡风轻：https://gocos.cn', title: '超星网课助手提示', btn: '我已知悉', offset: 't', closeBtn: 0});
    });
...
function newsubmit(){
    (function() {
        ...
        var a = {};
        a.sbchaoxing = {};
        a.sbchaoxing.staticRemoteHost = "https://api.gocos.cn/index.php/cxapi/cxtimu/lr";
        a.sbchaoxing.goNext = function() {

            if ($(".ZyTop h3 span").text().trim().indexOf("已完成") != -1) {
                $("#toNext1").text("Answer updated……");
                window.setTimeout(function() {
                        if ($(top.document).find(".roundpointStudent").index($(".roundpointStudent")) != $(top.document).find(".roundpointStudent").size()) {
                            ...
                            $("#toNext1").text("Next episode not found");
                        }
                    },
                    5000);
            }
        ...
        a.sbchaoxing.initUIexam = function() {
            var f =
                '<div style="border: 2px dashed rgb(149, 252, 251); width: 390px; min-height: 100px; max-height: 500px; font-size: 12px; text-align: left; position: fixed; top:10%; right:10%; z-index: 9999; background-color: rgba(184, 247, 255, 0.6);overflow: auto;">' +
                '<a style="text-decoration: none;font-size: large;width: 350px;display: block;float: left;" id="toNext1" href="javascript:">Answer updating......</a>' +
                '<button style="background-color: rgb(184, 247, 255);border: 0px #ffffff solid;text-decoration: none;font-size: large;width: 115px;display: block;float: left;" href="javascript:location.reload()" onclick="location.reload()">' +
                '重新查询' +
                '</button>' +

                '<button style="background-color: rgb(184, 247, 255);border: 0px #ffffff solid;text-decoration: none;font-size: large;width: 115px;display: block;float: left;" href="javascript:location.reload()" onclick="location.reload()">' +
                '<a style="text-decoration: none;" target="_blank" href="http://wk.gocos.cn">网课代看</a>' +
                '</button>' +
                '<button style="background-color: rgb(184, 247, 255);border: 0px #ffffff solid;text-decoration: none;font-size: large;width: 115px;display: block;float: left;" href="javascript:location.reload()" onclick="location.reload()">' +
                '<a style="background: url(http://hnkjxy.tsk.erya100.com/resource/images/student/baocun2.gif);border:0px;text-decoration: none;font-size: large;width: 100px;display: block;float: right;" href="javascript:void(0)" id="zhedie">折叠面板</a>' +
                '</button>' +
                '<table width="100%" id="antable" border="1">' +
                '<tr><td width="60%">题目</td><td width="40%">答案</td></tr>' +
                '</table>' +
                '</div>';
            $("body").append(f);
                    ...
                    if ($(h).find(".Py_answer.clearfix i.fr").size() > 0) {
                        ...
                        if ($(h).find(".Py_answer.clearfix i.fr.dui").size() > 0) {
                            ...
                            if (!p)
                            {
                                ...
                                {
                                    var f = $(h).find(".Py_tk div.clearfix");
                                    f.each(function(i, q) {
                                        j += $(this).text().trim();
                                        //alert(j);
                                        if (i < f.size() - 1) {
                        ...
                        var p = $(h).find(".Py_tk:first div .font14:first").text().trim();
                ...
                if (p.indexOf("我的答案") == -1) {
                    ...
                    $.ajax({
                        ...
                        error: function(XMLHttpRequest, textStatus, errorThrown) {
                            ...
                            $("#antable").append("<tr><td style='overflow:hidden;'>" + o + "</td><td>" + p + "-参数不全或者服务器炸了...</td></tr>");
                        }
                    });
                } else {
...
function beforeFind() {
    ...
    setting.div = $(
        ...
        "<button><a href='http://wk.gocos.cn' target='_blank' style='text-decoration: none;color:black;'>免费网课代看</a></button>" +
...
function findAnswer() {
    ...
    if (setting.num >= $('.TiMu').length) {
        var arr = setting.lose ? ['共有 <font color="red">' + setting.lose + '</font> 道题目待完善（已深色标注）', saveThis] : ['答题已完成&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://wkjd.52msr.cn" target="_blank">网课代看</a>', submitThis];
    ...
    GM_xmlhttpRequest({
        ...
        data: 'question=' + encodeURIComponent(question),// + "&token=" + setting.token,
...
function submitAnswer($job, data) {
    ...
    data.length && GM_xmlhttpRequest({
        method: 'POST',
        url: 'https://api.gocos.cn/index.php/cxapi/cxtimu/index',
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
